...
final class mongo_db {
	...
	public function connect() {
		...
		if (!$connect_options['username'])
			{	unset( $connect_options['username']);	}
		if (!$connect_options['password'])
			{	unset ( $connect_options['password']);	}
		if (!$connect_options['replicaSet'])
			{	unset ($connect_options['replicaSet']); }
		
	
		$connection_str	= "mongodb://{$this->config['hostname']}:{$this->config['port']}";
		
		try {
			//if(!$this->link = @new Mongo($connection_str, $connect_options)) {
			$this->link = new Mongo($connection_str,$connect_options);
	...
	public function remove($collection_name, $criteria, $option=array("justOne"=>false)){
		$collection = $this->selectCollection($collection_name);
		return $collection->remove($criteria, $option);
	}
